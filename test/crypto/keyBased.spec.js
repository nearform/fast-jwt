...
for (const method of ['RS', 'PS', 'ES']) {
  const privateKey = readFileSync(resolve(process.cwd(), `./test/fixtures/keys/${method.toLowerCase()}/private.key`))
  const publicKey = readFileSync(resolve(process.cwd(), `./test/fixtures/keys/${method.toLowerCase()}/public.key`))
